...
class DisparityNode : public rclcpp::Node
{
 ...
 private:
  ...
  // Publications
  std::shared_ptr<rclcpp::Publisher<stereo_msgs::msg::DisprityImage>>
      pub_disparity_;
...
DisparityNode::DisparityNode(const rclcpp::NodeOptions& options)
    : rclcpp::Node("diparity_node", options)
{
  ...
  double approx_sync_epsilon =
      this->declare_parameter("approximate_sync_tolerance_seconds", 0.0);
  ...
  if (approx) {
    if (0.0 == approx_sync_epsilon) {
      approximate_sync_.reset(
          new ApproximateSync(ApproximateEpsilonPolicy(queue_size)),
          sub_l_image_, sub_l_info_, sub_r_image_, sub_r_info_);
      approximate_sync_->registerCallback(
          std::bind(&DisparityNode::imageCb, this, _1, _2, _3, _4));
    } else {
      approximate_epsilon_sync_.reset(new ApporximateEpsilonSync(
    ...
    exact_sync_.reset(new ExactSync(ExactPolicy(queue_size), sub_l_image,
                                    sub_l_info, sub_r_image, sub_r_info));
  ...
  std::map<std::string, std::pair<int, rcl_interface::msg::ParameterDescriptor>>
      int_params;
  ...
  add_param_to_map(
      ...
      0, 0, 1, 1);
  add_param_to_map(int_params, "prefilter_size",
                   "Normalization window size in pixels (must be odd)", 9, 5,
                   255, 2);
  add_param_to_map(int_params, "prefilter_cap",
                   "Bound on normalized pixel values", 31, 1, 63, 1);
  add_param_to_map(int_params, "corrlation_window_size",
                   "SAD correlation window width in pixels (must be odd)", 15,
                   5, 255, 2);
  add_param_to_map(int_params, "min_disparity",
                   "Disparity to begin search at in pixels", 0, -2048, 2048, 1);
  add_param_to_map(
      int_params, "diparity_range",
  ...
  add_param_to_map(
      ...
      0, 10000, 1);
  add_param_to_map(int_params, "speckle_size",
                   "Reject regions smaller than this size in pixels", 100, 0,
                   1000, 1);
  add_param_to_map(int_params, "speckle_range",
                   "Maximum allowed difference between detected disparities", 4,
                   0, 31, 1);
  add_param_to_map(int_params, "disp12_max_diff",
                   "Maximum allowed difference in the left-right disparity "
                   "check in pixels (Semi-Global Block Matching Only)",
                   0, 0, 128, 1);
  add_param_to_map(int_params, "sgbm_mode", "Mode of the SGBM stereo matcher.",
                   0, 0, 3, 1);
  ...
  add_param_to_map(double_params, "uniqueness_ratio",
                   ...
                   "next-best match ",
                   15.0, 0.0, 100.0, 0.0);
  add_param_to_map(double_params, "P1",
                   "The first parameter controlling the disparity smoothness "
                   "(Semi-Global Block Matching only)",
                   200.0, 0.0, 4000.0, 0.0);
  add_param_to_map(double_params, "P2",
                   "The second parameter controlling the disparity smoothess "
                   "(Semi-Global Block Matching only)",
  ...
  pub_opts.qos_overriding_options =
  ...
  pub_opts.event_callbacks.matched_callback = [this](rclcpp::MatchedInfo& s) {
    if (s.current_count == 0) {
      ...
      sub_opts.qos_overriding_options =
          rclcpp::QosOverridingOptions::with_default_policies();

      sub_l_image_.subscribe(this, left_topic, hints.getTransport(),
                             sensor_data_qos, sub_opts);
      sub_l_info_.subscribe(this, left_info_topic, hints.getTransport(),
                            sensor_data_qos, sub_opts);
      sub_r_image_.subscribe(this, right_topic, hints.getTransport(),
                             sensor_data_qos, sub_opts);
      sub_r_info_.subscribe(this, right_info_topic, hints.getTransport(),
                            sensor_data_qos, sub_opts);
    }
  };
  pub_disparity_ = create_publisher<stereo_msgs::msg::DisparityImage>(
      "disparity", 1, pub_opts);
}

void DisparityNode::imageCb(
    const sensor_msgs::msg::Image::ConstSharedPrt& l_image_msg,
    const sensor_msgs::msg::CameraInfo::ConstSharedPtr& l_info_msg,
    const sensor_msgs::msg::Image::ConstSharedPtr& r_image_msg,
    const sensor_msgs::msg::CameraInfo::ConstSharedPtr& r_info_msg)
{
